...
class Enhanced4AgentRAG:
    ...
    def __init__(
        ...
        self.zaiAgent = ScadsApiAgent("zai-org/GLM-5.2-FP8")
        self.gptOssAgent = ScadsApiAgent("openai/gpt-oss-120b")


        self.question_splitter = QuestionSplitter(self.gptOssAgent, logger)
    ...
    def _process_single_question(self, query: str, goldGroundTruthPapers = None,db=None) -> Tuple[List[Tuple], List]:
        ...
        with time_block(f"documentEvaluation{query[:20]}"):
            ...
            for abstract_text, doc_id, answer in tqdm(doc_answers):
                ...
                text_answer, log_probs = self.gptOssAgent.generate(prompt, True)
...
def main():
    ...
    for i, item in enumerate(questions):
        questionAnchor = item["anchorPaper"]

        
        if (questionAnchor) in alreadyAnsweredQuestion:
            logger.info("already answered question with anchor" + questionAnchor + " - skipping")
            continue

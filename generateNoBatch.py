...
def generate_with_scads_api(finished_prompt_string):
    # 1. Initialize the client using the ScaDS API base URL
    # Replace os.getenv("SCADS_API_KEY") with your actual API key variable name
    client = OpenAI(
    ...
    try:
        # 2. Call the API
        print("Sending request to ScaDS 120B model...")
        response = client.chat.completions.create(
            model="openai/gpt-oss-120b",
            messages=[
                {"role": "user", "content": finished_prompt_string}
            ],
            temperature=0.0, # 0.0 is best for strict JSON parsing/judging
            max_tokens=2048, # Adjust based on how long your expected JSON is
            
            # Optional: If you strictly want the 120B model and want it to fail 
            # rather than falling back to a 70B model during high traffic:
...
def generateQuestion(arxivId, allSquaiArxivIds, db): #, judgingAgent
    ...
    print("asking llm")
    llmanswer = generate_with_scads_api(prompt)
    time.sleep(10)
